...
{
  FLASH_Status flashstatus = SP_FLASH_COMPLETE;
  ...
  {  
    ...
    { 
      flashstatus = SP_FLASH_ERROR_WRP;
    }
    else
    {
      if((FLASH->SR & FLASH_FLAG_RDERR) != (uint32_t)0x00)
      { 
        flashstatus = SP_FLASH_ERROR_RD;
      } 
      else 
      {
        if((FLASH->SR & (uint32_t)0xE0) != (uint32_t)0x00)
        {
          flashstatus = SP_FLASH_ERROR_PROGRAM;
        }
        else
        {
          if((FLASH->SR & FLASH_FLAG_OPERR) != (uint32_t)0x00)
          {
            flashstatus = SP_FLASH_ERROR_OPERATION;
          }
          else
          {
            flashstatus = SP_FLASH_ERROR_OPERATION;
...
/**
  ...
  *                       FLASH_ERROR_WRP, FLASH_ERROR_OPERATION or FLASH_COMPLETE.
  */
FLASH_Status FLASH_WaitForLastOperationSP(void)
{ 
  __IO FLASH_Status status = SP_FLASH_ERROR_OPERATION;
